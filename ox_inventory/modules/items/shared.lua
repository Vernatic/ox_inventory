...
local function newItem(data)
	...
	if isServer then
		...
		if clientData?.image then
			clientData.image = clientData.image:match('^[%w]+://') and clientData.image or
				('%s/%s'):format(client.imagepath, clientData.image)
		end
	end
...
for type, data in pairs(lib.load('data.weapons')) do
	for k, v in pairs(data) do
		...
		if isServer then
			...
			if clientData?.image then
				clientData.image = clientData.image:match('^[%w]+://') and ('url(%s)'):format(clientData.image) or
					('url(%s/%s)'):format(client.imagepath, clientData.image)
			end
		end
