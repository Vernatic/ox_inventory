			...
			if consume and consume ~= 0 and not data.component then
				...
				if durability then
					...
					if data.count > 1 then
						...
						if emptySlot then
							local newItem = Inventory.SetSlot(inventory, item, 6, table.deepclone(data.metadata),
								emptySlot)
...
exports('GetPlayerEquipment', function(playerId)
	local result = {}
	if #Player(playerId).state.playerEquipment > 0 then
		result = Player(playerId).state.playerEquipment
	else
		for i = 1, 8, 1 do
			result[i] = {
				slot = 1
...
local hookId = exports.ox_inventory:registerHook('swapItems', function(payload)
	...
	if payload.action == "move" and payload.toType == "drop" then 
		if not CFG.Drop.list[payload.fromSlot.name] then 
			return false
		end
	end
end, {})