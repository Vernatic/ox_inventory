...
export const getTargetInventory = (
  ...
  // Determine sourceInventory based on sourceType
  if (sourceType === InventoryType.PLAYER) {
    sourceInventory = state.leftInventory;
  } else if (sourceType === InventoryType.PLAYEREQUIPMENT) {
    sourceInventory = state.playerEquipment;
  } else {
    sourceInventory = state.rightInventory;
  }

  // Determine targetInventory based on targetType or fallback to opposite of sourceType
  if (targetType) {
    if (targetType === InventoryType.PLAYER) {
      targetInventory = state.leftInventory;
    } else if (targetType === InventoryType.PLAYEREQUIPMENT) {
      targetInventory = state.playerEquipment;
    }
    else {
