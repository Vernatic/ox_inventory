...
local function checkDropable(item)
	local result = false
	if CFG.Drop.type == 'blacklist' then
		result = true
	end
	if CFG.Drop.list[item] then
		result = not result
	end
	return result
end

local function checkGiveable(item)
	local result = true
	if CFG.Give.type == 'blacklist' then
		result = false
	end
	if not CFG.Give.list[item] then
		result = not result
	end
	return result
end
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	local UsableItems = lib.callback.await('ox_inventory:getUsableitemsData', 200)
	for _, v in pairs(Items --[[@as table<string, OxClientItem>]]) do
		...
		ItemData[v.name] = {
			...
			giveable = v.giveable or checkGiveable(v.name),
			dropable = v.dropable or checkDropable(v.name),
			usable = v.usable or (UsableItems[v.name] or false)
		}
	end
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	if data.toType == 'playerEquipment' or data.fromType == 'playerEquipment' then
		cb(false)
		return
	end
	swapActive = true
...
exports('GetPlayerEquipment', function()
	local result = {}
	if #plyState.playerEquipment > 0 then
		result = plyState.playerEquipment
	else
		for i = 1, 8, 1 do
			result[i] = {
				slot = 1
...
RegisterNUICallback('updateEquipment', function(data, cb)
	plyState:set('playerEquipment', data.items, true)
	cb(true)
end)

RegisterNUICallback('onClickWizardSkill', function(_, cb)
	print('onClickWizardSkill')
	cb(true)
end)

RegisterNUICallback('onClickSkillMeanage', function(_, cb)
	print('onClickSkillMeanage')
	cb(true)
end)
